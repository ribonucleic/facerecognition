...
def create_optimization_updates(cost, params, updates=None, max_norm = 5.0, lr = 0.01, eps= 1e-6, rho=0.95, method = "adadelta"):
	...
	gprevs  = [theano.shared(np.zeros_like(param.get_value(borrow=True))) if method == 'momentum' else None for param in params]

	c_prev = theano.shared(np.float64(1e100).astype(theano.config.floatX))
	...
	for gparam, param, gsum, xsum, gprev in zip(gparams, params, gsums, xsums, gprevs):
		...
		if method == 'adadelta':
			...
			dg = rho * gprev - (1-rho) * lr * gparam
			updates[param] = param + dg
			updates[gprev] = dg
			updates[lr] = lr *0.999
		else:
